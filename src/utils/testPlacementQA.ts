...
export function runPlacementQA(): void {
  console.log('🧪 Running Placement QA Tests...\n');
  ...
  TEST_CASES.forEach((testCase, index) => {
    ...
    console.log(`📊 Normalized scores: P:${normalized.pronunciation.toFixed(2)} G:${normalized.grammar.toFixed(2)} F:${normalized.fluency.toFixed(2)}`);
    
    // Determine placement
    const placement = determinePlacement(normalized);
    console.log(`🎯 Placement guest -> ${placement.level} | ${placement.reasoning} | answers:${testCase.answers}`);
    
    // Check if matches expected
    const passed = placement.level === testCase.expectedLevel;
    console.log(`✅ ${passed ? 'PASS' : 'FAIL'}: Expected ${testCase.expectedLevel}, got ${placement.level}`);
